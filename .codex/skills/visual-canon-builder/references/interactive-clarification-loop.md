...
```yaml
canon_assertions:
  - id: ASSERT_SampleSkate_StarEyes
    subject: CHR_SampleSkate
    predicate: hasEyeMotif
    object: black_eyes_with_white_star_pupils
    source_image_id: Image_SampleSkate_001
    source_role: approved canon source
    confidence: user_confirmed
    canon_status: confirmed
    approval_status: approved
    evidence_refs:
      - EV_SampleSkate_Face_001
    retrieval_scope: current_conversation_only
    asserted_by: user
    derived_from:
      - image_analysis_sample_skate_001
      - UA_SampleSkate_001
...
```yaml
relations:
  - subject: UA_SampleSkate_001
    predicate: answersQuestion
    object: Q_SampleSkate_001
  - subject: ASSERT_SampleSkate_StarEyes
    predicate: wasDerivedFrom
    object: UA_SampleSkate_001
```
...
Question Queue:
Q_SampleSkate_001 [ready-blocking]: Image_SampleSkate_001을 approved canon source로 승격할까요?
Q_SampleSkate_002 [ready-blocking]: 티셔츠 문구를 항상 정확히 유지해야 하나요?
Q_SampleSkate_003 [optional]: 말차 라떼와 스케이트보드는 고정 소품인가요?
...
- UA_SampleSkate_001 answered Q_SampleSkate_001: approve_as_canon
